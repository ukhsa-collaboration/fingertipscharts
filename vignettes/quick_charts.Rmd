...
The area profiles image is created using the `area_profiles()` function. This section shows how to produce a spine chart with the same data as in used in the [Overview](#overview) section, but we need to include the `rank = TRUE` parameter as this returns a polarity field, which is needed for the spine chart:
